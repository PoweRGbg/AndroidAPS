...
public class DetermineBasalAdapterSMBJS {
    ...
    public void setData(Profile profile,
        ...
        // use real values of COB and bolus to enable SMB
        boolean smbWithCOB = false;
        boolean smbWithBolus = false;
        boolean smbWithTemp = false;
        int bolusTimeMinutes = (int) (System.currentTimeMillis() - mealData.lastBolusTime)/(1000*60);
        log.debug("SMB-lastBolusTime was "+bolusTimeMinutes+" minutes ago");
        if(SP.getBoolean(R.string.key_use_smb,false) && mealData.mealCOB > 0) smbWithCOB = true;
        if(SP.getBoolean(R.string.key_use_smb,false) && mealData.boluses > 0) smbWithBolus = true;
        if(SP.getBoolean(R.string.key_use_smb,false) && tempTargetSet) smbWithTemp = true;
        mProfile.add("enableSMB_with_bolus", smbWithBolus);
        mProfile.add("enableSMB_with_COB", smbWithCOB);
        mProfile.add("enableSMB_with_temptarget", smbWithTemp);
        mProfile.add("maxSMBBasalMinutes", SP.getInt("key_smbmaxminutes",30));
        ...
        //mMealData.add("minDeviationSlope", mealData.minDeviationSlope); -- not used anymore
        mMealData.add("slopeFromMaxDeviation", mealData.slopeFromMaxDeviation);
        mMealData.add("slopeFromMinDeviation", mealData.slopeFromMinDeviation);
        mMealData.add("lastBolusTime", mealData.lastBolusTime);
        mMealData.add("lastCarbTime", mealData.lastCarbTime);
